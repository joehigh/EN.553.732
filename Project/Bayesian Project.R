...
library(lme4)
library(plyr)
library(dplyr) # Note -- I rely heavily on this package for data manipulation! 
               # Would strongly encourage you to review some of the documentation, 
               # it's the most useful R package
library(magrittr)
library(coda)
library(beepr)
...
mlb.analysis <- mlb %>% select(playerID,birthYear,weight,height,yearID,teamID,lgID,G,AB,H) %>% # Extract relvant variables...
  ...
  na.omit %>% # Drop records with missing information 
              # (a small number of players with no height/weight info from early 1900s)
  as.data.frame()
...
# Overly predictions on data. Less variable, which is what we would expect -- 
# predictions do not include residual variance
...
t <- Sys.time()
beta.samples<-coda.samples(avg.jags.model, variable.names = jags.parameters, n.iter = 5000,thin = 5)
Sys.time() - t
beep()
...
t <- Sys.time()
avg.jags.samples<-coda.samples(avg.jags.model, variable.names = jags.parameters, n.iter = 5000,thin = 5)
Sys.time() - t
beep()
